...
```{r}
# group level response, subject level predictor
n1 = 200
n2 = 15
xdata <- data.frame(x1 = rnorm(n1), group = sample(1:n2, n1, replace = TRUE))

xdata$groupintercept <- rnorm(n1)[xdata$group]
xdata$groupslope <- rnorm(n1)[xdata$group]

xdata$resp <- 2 + xdata$groupintercept +
  (1.5 + xdata$groupslope) * xdata$x1 +
  rnorm(n1)

xdata$resp2 <- tapply(xdata$resp, xdata$group, mean)[xdata$group]
res <- lmer(resp ~ x1 + (x1||group), data = xdata, REML = FALSE)
summary(res)
```
